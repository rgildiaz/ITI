...
This document will walk through the process I used to wipe 50 Intel NUC's by setting up a PXE server to network boot ShredOS. Section 1 will cover different methods I tried for setting up a PXE server, including Windows ADK and SLAX. Section 2 will document how I successfully configured a PXE server in [Ubuntu Server 20.04.4 LTS](https://releases.ubuntu.com/20.04/). The last section will cover workarounds I needed to use in some cases.

#### Contents:
- [0: Setup and Background](#0-setup-and-background)
- [1: Windows ADK and SLAX](#1-windows-adk-and-slax)
- [2: Ubuntu and PXELINUX](#2-ubuntu-and-pxelinux) - **skip here for setup instructions**
- [3: Workarounds and Issues](#3-workaroundsissues)
- [Conclusion: Running this Server in the Future](#conclusion-running-this-server-in-the-future)

---

## 0: Setup and Background
### Equipment
...
#### 2. DHCP
- **DHCP** is an acronym for [_Dynamic Host Configuration Protocol_](https://en.wikipedia.org/wiki/Dynamic_Host_Configuration_Protocol). It is a protocol for dynamically automatically assigning and managing IP addresses within a network. 
...
- DHCP allows for communication between server and client, and TFTP is the way in which they communicate.
...
## 1: Windows ADK and SLAX
### Windows ADK
After some initial research, I found this document about [configuring a PXE server to load Windows PE](https://docs.microsoft.com/en-us/windows/deployment/configure-a-pxe-server-to-load-windows-pe). This gave me hope that I might be able to use the [Windows Assessment and Deployment Kit (ADK)](https://docs.microsoft.com/en-us/windows-hardware/get-started/adk-install) to deploy a PXE boot server from my laptop, distributing ShredOS instead of Windows PE.

After [installing the Windows ADK](https://docs.microsoft.com/en-us/windows-hardware/get-started/adk-install), I quickly realized that it would only really help with deploying Windows over a PXE boot server.

Another issue with this solution is that the prerequisites include both a working DHCP server and a PXE server, neither of which I had or knew how to setup.

### SLAX
After asking Matthew for some help getting started, he gave me a bootable USB with [SLAX](https://www.slax.org/) on it. 
...
In the comments of this blog post thread, I found mention of ``pxelinux.0`` files. After researching PXELINUX (below) I thought it might make more sense to setup a PXE server using PXELINUX from scratch rather than trying to dig into the files I didn't understand within SLAX.
...
I tried following these instructions in SLAX, using the console to install ``dnsmasq`` and ``tftpd-hpa``.

---

## 2: Ubuntu and PXELINUX
...
## 3: Workarounds/Issues
### Some NUC's cannot PXE boot ShredOS.
Once I had the PXE boot server setup, I began plugging in as many NUC's as I could to get them all started at once. However, I found that about half of them came accross this error:
