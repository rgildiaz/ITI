...
For example, ``eth1`` interface with a VID of 100 would look like this:
```
auto eth1.100
...
```
interface eth1.100
static ip_address=w.x.y.z
```

For the changes to take effect, either restart netoworking services or restart the Pi:
```
sudo systemctl restart networking
```

In our case, see [the list of Pi hostnames and IP's used here](rpi-list.txt) to see how each Pi was configured.
> Since the hostnames are different for each Pi, I set these during the [post-imaging configuration](#post-image-configuration).
